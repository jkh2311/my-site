...
function render(list){var el=document.getElementById('parking-data-list');if(!el)return; if(!list||!list.length){el.innerHTML='';return}
 el.innerHTML=list.map(function(p){
   var map='https://www.google.com/maps/search/?api=1&query='+encodeURIComponent(p.name+' '+p.address);
   ...
   '<p class="parking-map-links"><a href="'+map+'" target="_blank" rel="noopener">구글지도 위치 보기</a><a href="'+naver+'" target="_blank" rel="noopener">네이버지도에서 보기</a></p></article>'
 }).join('');
